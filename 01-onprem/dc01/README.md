...
│   ├─ Admin      → user3\_admin (Sec\_Admins)

│   ├─ HR         → user2\_hr (Sec\_HR)

│   ├─ IT         → user1\_it (Sec\_IT)

│   └─ General    → user4\_general
...
├─ Admin\_NoSync   → user3\_admin, App01Admin (excluded from Entra sync)

└─ Service\_Accounts (reserved for service accounts)
...
| user1\_it | IT | IT Technician | user1\_it@daniel.local |

| user2\_hr | HR | HR Specialist | user2\_hr@daniel.local |

| user4\_general | General | General User | user4\_general@daniel.local |
...
| Sec\_Admins | user3\_admin | Contributor — rg-daniellab |

| Sec\_IT | user1\_it | Reader — rg-daniellab |

| Sec\_HR | user2\_hr | Reader — rg-daniellab |
